...
ENV PREFIX="/usr/local" ENV PKG_CONFIG_PATH="$PREFIX/lib/pkgconfig:$PREFIX/lib/x86_64-linux-gnu/pkgconfig" ENV CFLAGS="-O3 -march=znver2 -mtune=znver2 -flto -ffunction-sections -fdata-sections -fomit-frame-pointer" ENV CXXFLAGS="$CFLAGS" ENV LDFLAGS="-static -flto -Wl,--gc-sections"

FROM base AS build-core

WORKDIR /build/zlib RUN curl -sSL https://zlib.net/zlib-1.3.1.tar.gz -o zlib.tar.gz && 
tar --strip-components=1 -xzf zlib.tar.gz && 
./configure --prefix=$PREFIX --static && make -j$(nproc) && make install

WORKDIR /build/x264 RUN git clone --depth=1 https://code.videolan.org/videolan/x264.git . && 
...
WORKDIR /build/x265 RUN git clone https://bitbucket.org/multicoreware/x265_git . && 
cd build/linux && 
...
WORKDIR /build/libvpx RUN git clone --depth=1 https://chromium.googlesource.com/webm/libvpx . && 
...
WORKDIR /build/aom RUN git clone --depth=1 https://aomedia.googlesource.com/aom . && 
...
WORKDIR /build/fdk-aac RUN git clone --depth=1 https://github.com/mstorsjo/fdk-aac.git . && 
autoreconf -fiv && 
./configure --prefix=$PREFIX --disable-shared --enable-static 
CFLAGS="$CFLAGS" LDFLAGS="$LDFLAGS" && 
make -j$(nproc) && make install

WORKDIR /build/lame RUN wget https://downloads.sourceforge.net/project/lame/lame/3.100/lame-3.100.tar.gz && 
tar xzf lame-3.100.tar.gz --strip-components=1 && 
./configure --prefix=$PREFIX --disable-shared --enable-static --enable-nasm 
CFLAGS="$CFLAGS" LDFLAGS="$LDFLAGS" && 
make -j$(nproc) && make install

WORKDIR /build/libogg RUN wget https://downloads.xiph.org/releases/ogg/libogg-1.3.6.tar.gz && 
tar xzf libogg-1.3.6.tar.gz --strip-components=1 && 
./configure --prefix=$PREFIX --disable-shared --enable-static 
CFLAGS="$CFLAGS" LDFLAGS="$LDFLAGS" && 
make -j$(nproc) && make install

WORKDIR /build/xvidcore RUN wget https://downloads.xvid.com/downloads/xvidcore-1.3.7.tar.gz && 
...
WORKDIR /build/ffmpeg RUN git clone --depth=1 https://github.com/FFmpeg/FFmpeg.git . RUN ./configure 
--prefix=$PREFIX 
...
--enable-inline-asm --enable-x86asm RUN make -j$(nproc) && make install && strip $PREFIX/bin/ffmpeg

FROM scratch COPY --from=build-core /usr/local/bin/ffmpeg /ffmpeg
